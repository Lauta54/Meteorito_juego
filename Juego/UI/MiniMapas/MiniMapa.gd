...
onready var timer_visibilidad:Timer = $TimerVisivilidad
...
func _ready() -> void:
	...
	escala_grilla = zona_renderizado.rect_size / (get_viewport().size * escala_zoom)
	conectar_seniales() 
...
func modificar_posicion_iconos() -> void:
	for item in items_mini_mapa:
		...
		zona_renderizado.rect_position):
...
## Señales Internas
func _on_TimerVisivilidad_timeout() -> void:
	if esta_visible:
