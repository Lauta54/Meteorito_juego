...
func _on_actualizar_info_tiempo(tiempo_restante:int) -> void:
	var minutos:int = floor(tiempo_restante * 0.0166666666667)
	...
	if tiempo_restante % 10 == 0:
		info_tiempo_restante.mostar_suavizado()
...
func _on_actualizar_energia_laser(enegia_max: float, 
energia_actual:float) -> void:
	info_laser.mostrar()
	info_laser.actualizar_energia(enegia_max, energia_actual)
